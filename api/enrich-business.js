export default async function handler(req,res){
  ...
  try{
    ...
    const facts=[];
    const push=(field,value,source='texto fornecido')=>{if(value&&!facts.some(x=>x.field===field&&x.value===value))facts.push({field,value:clean(value),source,status:'fornecido'});};
    ...
    // Optional provider integrations. No data is invented when credentials are absent.
    if(process.env.GOOGLE_PLACES_API_KEY&&(businessName||raw)){
      ...
      if(r.ok){const j=await r.json();(j.places||[]).forEach((p,i)=>{const src=`Google Places #${i+1}`;if(p.displayName?.text)push('nome',p.displayName.text,src);if(p.formattedAddress)push('endereço',p.formattedAddress,src);if(p.nationalPhoneNumber||p.internationalPhoneNumber)push('telefone',p.nationalPhoneNumber||p.internationalPhoneNumber,src);if(p.websiteUri)sourceResults.push({url:p.websiteUri,type:'site oficial provável',status:'encontrado',source:src});if(p.rating)push('avaliação',String(p.rating),src);if(p.userRatingCount)push('número de avaliações',String(p.userRatingCount),src);if(p.googleMapsUri)sourceResults.push({url:p.googleMapsUri,type:'Google Maps',status:'encontrado',source:src});});}
    }
  ...
  }catch(e){console.error(e);return res.status(500).json({error:'Não foi possível processar as informações.',detail:e.message});}
}
